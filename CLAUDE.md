...
- [ ] Install `pdfkit` package + `@types/pdfkit` devDependency for server-side PDF generation
...
- [ ] `GET /api/analytics/export-pdf` endpoint — auth + rate limit (10/hr per user); accepts `?period=7d|30d|90d` (default 30d); compiles data from DB (post counts by status, PublishResult platform breakdown, PostInsights engagement, top posts by score); calls `generateAnalyticsPdf`; streams response with `Content-Type: application/pdf` and `Content-Disposition: attachment; filename="postflow-report-{date}-{period}.pdf"`; logs activity `analytics.exported`
- [ ] "Export PDF" button (`FileDown` icon) in analytics dashboard header — calls `/api/analytics/export-pdf` with current period param via anchor tag download; shows loading spinner while fetching; toast on error
- [ ] Unit tests for PDF export endpoint (auth 401, rate limit 429, invalid period fallback to 30d, returns PDF content-type, content-disposition header, response body starts with `%PDF`, activity logged — 8 tests)
