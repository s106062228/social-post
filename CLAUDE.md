...
- [ ] `SyndicationRule` model in Prisma (id, userId, name, sourcePlatform Platform, targetPlatforms Platform[], transformations Json (truncate?, appendHashtags?, stripLinks?, customSuffix?), delayMinutes Int default 0, isActive Boolean default true, createdAt, updatedAt) + migration (`20260828000000_add_syndication_rules`) — defines how content published to one platform gets automatically adapted and cross-posted to others
- [ ] CRUD API for syndication rules (`GET /api/syndication-rules`, `POST /api/syndication-rules`, `PATCH /api/syndication-rules/[id]`, `DELETE /api/syndication-rules/[id]`) — auth + rate limit + zod validation; max 20 rules per user; validate sourcePlatform ≠ targetPlatforms
- [ ] `POST /api/syndication-rules/[id]/test` endpoint — auth + rate limit + ownership check; accepts `{content: string, mediaType: MediaType}`; applies transformations and returns `{original, adapted: {platform, content}[]}` preview without creating posts
- [ ] Syndication utility (`src/lib/syndication.ts`) — `applyTransformations(content, transformations, targetPlatform)` truncates to platform char limit, strips URLs if stripLinks, appends customSuffix, appends hashtags; `getSyndicationRulesForPlatform(userId, platform)` fetches matching active rules
- [ ] Update BullMQ publish worker — after a successful PUBLISHED result, look up syndication rules matching the published platform; for each matching rule, create a new DRAFT/SCHEDULED post (scheduledAt = now + delayMinutes) with adapted content for each target platform; logs activity
- [ ] Syndication rules management page in dashboard (`/syndication`) — list rules with source→target platform arrow badges, delay chip, active/pause toggle; inline create form (name, source platform, target platforms multi-select, delay dropdown 0/15/30/60/120/240 min, transformation checkboxes + customSuffix input); delete with confirmation toast
- [ ] Add "Syndication" to sidebar navigation (icon: `Share2`)
- [ ] Unit tests for syndication utility and API endpoints (applyTransformations truncation, stripLinks, appendHashtags, customSuffix; GET list, POST create, POST max-limit, POST invalid same-platform, PATCH update, PATCH ownership, DELETE success, test endpoint preview — auth, rate limit, validation — 18 tests)
