...
### Phase 250: AI-Powered Post Performance Explanations & Natural Language Insights
- [x] Add `explainPostPerformance(content, insights, historicalAvg?, platform?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate a natural language explanation of a post's performance; accepts post content, aggregated insights (impressions/reach/likes/comments/shares), optional historical averages for comparison, and optional platform name; uses prompt caching on system block; returns `{explanation: string, keyFactors: {factor: string, impact: "positive"|"negative"|"neutral", description: string}[], actionItems: string[]}` or null when AI not configured
- [x] `POST /api/posts/[id]/explain-performance` endpoint — auth + rate limit + ownership; queries post with publishResults and PostInsights; aggregates insights across all publish results; computes 30-day historical averages for context; calls `explainPostPerformance`; returns `{explanation, keyFactors, actionItems}`; returns 503 when AI not configured; returns 422 when post has no published insights
- [x] `PerformanceExplanationCard` component (`src/components/performance-explanation-card.tsx`) — card with "Generate Explanation" button; after generation shows AI-written explanation text, color-coded key factors list (TrendingUp green=positive / TrendingDown red=negative / Minus grey=neutral each with factor name + description), numbered action items list; Regenerate button; loading state
- [x] Integrate `PerformanceExplanationCard` into the post versions page (`src/app/(dashboard)/posts/[id]/versions/page.tsx`) — imported and added below `<PostInsightsPanel postId={postId} />` and above `<SeoAnalysisCard postId={postId} />`
- [x] Unit tests for explain-performance endpoint (`src/lib/__tests__/explain-performance.test.ts` — 10 tests: auth 401, rate limit 429, not-found 404, no insights 422, AI disabled 503, success 200, explanation string, keyFactors shape, actionItems array, DB error 500)

### Phase 251: Social Media Text Formatter & Unicode Styling Tools
- [x] Text formatting utility (`src/lib/text-formatter.ts`) — `formatText(text, style)` converts plain text to Unicode mathematical bold (𝗮-𝘇), italic (𝘢-𝘻), bold-italic, strikethrough (combining U+0336), and monospace (𝚊-𝚣) styles; digit support for bold and monospace; `convertEmojiShortcodes(text)` replaces 46 `:shortcode:` patterns (`:fire:` → 🔥, `:rocket:` → 🚀, etc.); `processText(text, style, convertEmojis)` pipeline converts shortcodes first then applies style, returns `{formatted, charCount}` using Unicode code-point length
- [x] `POST /api/posts/format-text` endpoint — auth + rate limit + zod validation (`text: string min 1 max 10000`, `style: "bold"|"italic"|"bold-italic"|"strikethrough"|"monospace"|"none"`, `convertEmojis?: boolean`); calls `processText`; returns `{formatted, charCount}`
- [x] `TextFormatterBar` component (`src/components/text-formatter-bar.tsx`) — compact horizontal toolbar with B / I / BI / S̶ / Mono / :emoji: buttons; applies transformation client-side via `formatText` / `convertEmojiShortcodes` directly (no round-trip needed); integrated into post composer between the AI-tools toolbar and the content textarea; shows toast feedback; buttons disabled when content is empty
- [x] Unit tests for text formatter utility (`src/lib/__tests__/text-formatter.test.ts` — 26 tests: bold/italic/bold-italic/strikethrough/monospace transformations, digit handling, h italic exception, non-ASCII pass-through, punctuation pass-through, emoji shortcode conversion, unknown shortcodes unchanged, processText style+emoji pipeline, code-point char count)

### Phase 252: AI-Powered Trending Topic Discovery & Content Suggestions
- [x] Add `discoverTrendingTopics(niche, existingTopics, platforms)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to identify 5–8 trending topics in the user's niche that they should be creating content about now; returns `TrendingTopicsResult {topics: TrendingTopic[], generalInsights: string}` where each topic has `{topic, category, urgency: "now"|"this_week"|"this_month", reasoning, contentIdea, estimatedEngagement: "high"|"medium"|"low"}`; uses prompt caching on system block; returns null when AI not configured
- [x] `POST /api/ai/trending-topics` endpoint — auth + rate limit + zod validation (`niche?: string max 200`, `platforms: string[] min 1`); loads user's last 30 PUBLISHED posts for existing topic context; calls `discoverTrendingTopics`; returns `{topics, generalInsights}`; 503 when AI not configured or returns null
- [x] `TrendingTopicsCard` component (`src/components/trending-topics-card.tsx`) — client component with niche input and "Discover"/"Refresh" button; displays topics in a 2-column card grid with urgency badges (Trending Now=red/This Week=amber/This Month=blue), category chip, engagement indicator, reasoning text, content idea, and "Create post about this" link; shows "Strategy Insight" general insights banner; loading/empty states
- [x] Add `"trending_topics"` to `WIDGET_KEYS` array and `trending_topics: "Trending Topic Discovery"` to `WIDGET_LABELS` in `src/app/api/dashboard-widgets/route.ts`; integrate `TrendingTopicsCard` into analytics dashboard with `isVisible("trending_topics")` guard
- [x] Unit tests for `POST /api/ai/trending-topics` (auth, rate limit, AI key not set, invalid JSON, empty platforms, AI returns null, success with topics and generalInsights, platforms/niche forwarded to AI, existing topics extracted from posts, all topic fields present, works without niche — 11 tests)

### Phase 253: A/B Test Statistical Significance & Automated Winner Detection
- [x] Statistical significance utility (`src/lib/ab-stats.ts`) — exports `EngagementMetrics` and `ABStatResult` interfaces; `computeABStats(metricsA, metricsB)` implements two-proportion z-test: engagement rate = (likes+comments+shares)/impressions per variant, pooled SE, z-score, two-tailed p-value via Abramowitz & Stegun normal CDF approximation; `confidenceLevel` (90/95/99), `isSignificant` (≥95%), `winnerLead` ("A"|"B"|"INCONCLUSIVE"), `effect` (relative % difference), `hasSufficientData` (both impressions ≥100)
- [x] `GET /api/ab-tests/[id]/stats` endpoint — auth + rate limit + ownership check; aggregates publishResults.insights for postA and postB; calls `computeABStats`; returns `{testId, name, winner, createdAt, stats}`
- [x] BullMQ A/B test conclude worker (`src/lib/queue/workers/ab-test-conclude.ts`) — daily cron 09:00 UTC; scans all open PostABTests (winner=null) created ≥7 days ago; aggregates metrics; auto-concludes tests with `hasSufficientData && isSignificant` by setting winner to `winnerLead` with notes "Auto-concluded: {confidence}% confidence, Z-score: {z}"
- [x] Add `AB_TEST_CONCLUDE_SCAN: "postflow:ab-test-conclude-scan"` to `QUEUE_NAMES` in `connection.ts`; add `scheduleABTestConcludeScan()` to `scheduler.ts` (daily "0 9 * * *"); register worker + cron in `workers/queue-worker.ts`
- [x] `ABTestStatisticsPanel` client component (`src/components/ab-test-statistics-panel.tsx`) — fetches `GET /api/ab-tests/[id]/stats`; shows confidence badge, winner lead status, engagement rate comparison table (impressions, total engagements, rate %, effect size), z-score / p-value / data sufficiency stat boxes; loading/error states
- [x] Integrate `ABTestStatisticsPanel` into A/B test detail page (`src/app/(dashboard)/ab-tests/[id]/page.tsx`) above the ConcludeTestForm
- [x] Unit tests for ab-stats utility (10 tests — both-zero, hasSufficientData threshold, A wins, B wins, equal=INCONCLUSIVE, 95% confidence with large effect, not significant with small sample, effect size calculation, p-value in 0-1 range, rateA=0 when no impressions) and stats endpoint (8 tests — auth 401, rate limit 429, not-found 404, wrong user 403, no-insights zeroed result, aggregation shape, significant with large samples, response fields present)

### Phase 254: AI-Powered Product & Service Caption Generator
- [x] Add `generateProductCaptions(productName, productDescription, platforms, promotionType, targetAudience?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate platform-optimized captions for products/services; supports promotion types (launch/sale/awareness/review/general); uses prompt caching on system block; returns `{captions: [{platform, caption, tone, charCount}], keyMessages: string[]}`; returns null when AI not configured
- [x] `POST /api/ai/product-captions` endpoint — auth + rate limit + zod validation (`productName: string`, `productDescription: string min 10 max 2000`, `platforms: string[] min 1`, `promotionType?: enum`, `targetAudience?: string`); calls `generateProductCaptions`; returns `{captions, keyMessages}`; returns 503 when AI not configured; 500 when AI returns null
- [x] `ProductCaptionDialog` component (`src/components/product-caption-dialog.tsx`) — modal with product name input, description textarea (char counter), promotion type selector (General/Product Launch/Sale/Brand Awareness/Customer Review), optional target audience input; calls `/api/ai/product-captions`; shows per-platform caption cards with tone badge, character count vs limit (color-coded), copy-to-clipboard, "Use in Post" button; regenerate option; loading state
- [x] "Product" button (`ShoppingBag` icon) in post composer toolbar — opens `ProductCaptionDialog` with currently selected platforms; `onApply` sets post content
- [x] Unit tests for `POST /api/ai/product-captions` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, missing productName 400, description too short 400, empty platforms 400, success shape with captions/keyMessages, promotionType + targetAudience forwarded, default promotionType "general", AI returns null → 500, AI error → 500 — 12 tests)

### Phase 255: AI-Powered Audience Q&A Post Generator
- [x] Add `generateAudienceQuestions(topic, platforms, count, context?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate the most common audience questions for a topic along with ready-to-publish answers formatted as social media posts; returns `{questions: [{question, answer, suggestedPost, category}], topic}`; category is one of "how-to"|"why"|"what"|"comparison"|"misconception"|"tip"; uses prompt caching on system block; returns null when AI not configured
- [x] `POST /api/ai/audience-questions` endpoint — auth + rate limit + zod validation (`topic: string min 2 max 200`, `platforms: string[] min 1 max 10`, `count?: 1–10 default 5`, `context?: string max 500`); calls `generateAudienceQuestions`; returns `{questions, topic}`; returns 503 when AI not configured; 500 when AI returns null
- [x] `AudienceQuestionsDialog` component (`src/components/audience-questions-dialog.tsx`) — modal with topic input, optional context textarea, count selector (3/5/7/10); calls `/api/ai/audience-questions`; shows accordion of questions with category colour badges (How-To/Why/What/Comparison/Misconception/Tip), expandable answer + suggested post, copy-to-clipboard and "Use in Post" buttons per question; regenerate option; loading state
- [x] "Q&A" button (`HelpCircle` icon) in post composer toolbar — opens `AudienceQuestionsDialog` with currently selected platforms; `onApply` sets post content
- [x] Unit tests for `POST /api/ai/audience-questions` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, topic missing 400, topic too short 400, empty platforms 400, success shape with questions/topic, count + context forwarded, default count 5, AI returns null → 500, AI error → 500 — 12 tests)

### Phase 256: Post Engagement Milestone Notifications
- [x] `PostEngagementMilestone` model in Prisma (id, userId, postId, metric, threshold, achievedAt, celebrated, createdAt) + `@@unique([postId, metric, threshold])` + migration (`20260920000000_add_post_engagement_milestones`) — tracks when posts cross key engagement thresholds
- [x] BullMQ engagement milestone scan worker (`src/lib/queue/workers/engagement-milestones.ts`) — daily cron 10:00 UTC; scans all PUBLISHED posts with PostInsights; for each metric (impressions, reach, likes, comments, shares) × threshold combination that has been crossed, upserts a `PostEngagementMilestone` record; fires in-app notification via `createNotification` when a new milestone is detected
- [x] Add `ENGAGEMENT_MILESTONE_SCAN: "postflow:engagement-milestone-scan"` to `QUEUE_NAMES` in `connection.ts`; add `scheduleEngagementMilestoneScan()` to `scheduler.ts`; register worker + cron in `workers/queue-worker.ts`
- [x] `GET /api/analytics/engagement-milestones` endpoint — auth + rate limit + `?period=7d|30d|90d` (default 30d); returns up to 50 milestones ordered by achievedAt desc with post content preview
- [x] `POST /api/analytics/engagement-milestones/[id]/celebrate` endpoint — auth + rate limit + ownership check; sets `celebrated=true`; returns `{celebrated: true}`
- [x] `EngagementMilestonesCard` component (`src/components/engagement-milestones-card.tsx`) — period selector (7d/30d/90d); milestone list with metric icon, threshold badge, post content preview, achievement date, 🎉 celebrate button; loading skeleton; empty state
- [x] Add `"engagement_milestones"` widget key and label `"Engagement Milestones"` to `WIDGET_KEYS` and `WIDGET_LABELS` in dashboard-widgets route; integrate `EngagementMilestonesCard` into analytics dashboard
- [x] Unit tests for engagement milestones API (GET: auth 401, rate limit 429, returns milestones array, post info included, empty array, period filter, 30d default; POST celebrate: auth 401, rate limit 429, 404 not-found, 404 wrong-user, 200 success, prisma update called — 13 tests)

### Phase 257: Sponsored Post & Paid Partnership Disclosure Manager
- [x] Add `isSponsored` (Boolean default false) + `sponsorName` (nullable String) + `disclosureText` (nullable String) to Post model + Prisma migration (`20261001000000_add_sponsored_post`)
- [x] `PATCH /api/posts/[id]/sponsor` endpoint — auth + rate limit + ownership check; accepts `{isSponsored: boolean, sponsorName?: string, disclosureText?: string}`; validates that when isSponsored=true at least sponsorName or disclosureText is provided; updates post; returns `{isSponsored, sponsorName, disclosureText}`
- [x] Extend `GET /api/posts` to support `?sponsored=true` filter and include `isSponsored`, `sponsorName`, `disclosureText` in post responses
- [x] Disclosure utility (`src/lib/disclosure.ts`) — `addDisclosure(content, disclosureText, position: "prepend"|"append")` inserts disclosure into content with separator; `getDefaultDisclosure(platforms)` returns appropriate FTC disclosure (#ad for Twitter/Instagram, "Paid partnership" label for LinkedIn/Facebook); `buildDisclosureText(sponsorName, platforms)` constructs full disclosure string from sponsor name
- [x] Integrate disclosure into BullMQ publish worker — when `post.isSponsored=true` and content is being published, call `addDisclosure` with the stored disclosureText (if set) or `buildDisclosureText(sponsorName, [platform])` before passing content to platform adapter; runs only when disclosureText or sponsorName is present
- [x] `SponsorBadge` client component (`src/components/sponsor-badge.tsx`) — compact orange "Sponsored" pill badge; tooltip showing sponsorName when set; integrated into posts list row when `isSponsored=true`
- [x] `SponsorPostButton` client component (`src/app/(dashboard)/posts/sponsor-post-button.tsx`) — icon button per post row; opens dialog with isSponsored toggle, sponsorName input, disclosureText textarea (with char limit), and disclosure preview; calls PATCH endpoint on save; toast feedback; optimistic UI
- [x] "Sponsored" filter tab in posts list page alongside existing status/starred/evergreen/archived tabs
- [x] Unit tests for sponsor endpoint (auth, rate limit, ownership, set sponsored with name, set sponsored with disclosure, set sponsored without name/disclosure returns 400, clear sponsor, disclosure added to content in worker, default disclosure built from sponsor name — 12 tests)

### Phase 258: Tag Performance Analytics Dashboard
- [x] `GET /api/analytics/tag-performance` endpoint — auth + rate limit + `?period=7d|30d|90d|all&limit=`; queries user's PUBLISHED posts joined to PostTag/Tag records and PostInsights; groups by tag; computes postCount, avgEngagement (weighted: likes×3+comments×5+shares×4+reach+impressions×0.5), totalEngagement, totalLikes, totalComments, totalShares, totalReach, totalImpressions per tag; returns sorted by avgEngagement desc
- [x] `TagPerformanceCard` component (`src/components/tag-performance-card.tsx`) — ranked list of tags with coloured tag pill, post count badge, avg engagement progress bar, total likes/comments/shares chips; period selector (7d/30d/90d/all); loading skeleton; "No data yet" empty state
- [x] Integrate `TagPerformanceCard` into analytics dashboard below `EngagementMilestonesCard`; add `"tag_performance"` widget key and label `"Tag Performance Analytics"` to `WIDGET_KEYS` and `WIDGET_LABELS` in dashboard-widgets route; add to `DashboardCustomizeDialog`
- [x] Unit tests for tag-performance endpoint (auth 401, rate limit 429, invalid period 400, empty state, period echoed, defaults to 30d, sorted by avgEngagement desc, totalTaggedPosts count, response shape, limit param — 10 tests)

### Phase 259: AI Caption Style Transfer
- [x] Add `styleTransferContent(content, targetStyle, platforms)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to rewrite post content in the requested style while preserving core message, hashtags, and mentions; target styles: `casual`, `professional`, `concise`, `engaging`, `humorous`, `inspirational`, `educational`; uses prompt caching on system block; returns `{styledContent: string, changes: string[], styleName: string}` or null when AI not configured
- [x] `POST /api/ai/style-transfer` endpoint — auth + rate limit + zod validation (`content: string min 10 max 10000`, `targetStyle: enum`, `platforms: Platform[]`); calls `styleTransferContent`; returns `{styledContent, changes, styleName}`; returns 503 when AI not configured; 500 when AI returns null
- [x] `StyleTransferDialog` component (`src/components/style-transfer-dialog.tsx`) — modal with 7-style grid showing icon + name + description for each style; calls `/api/ai/style-transfer` on style select; shows before/after side-by-side comparison with character counts; "Apply" button sets composer content; "Regenerate" button re-runs; loading state
- [x] "Style" button (Palette icon) in post composer toolbar — opens `StyleTransferDialog` with current content and selected platforms; disabled when content is empty
- [x] Unit tests for `POST /api/ai/style-transfer` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, content too short 400, empty platforms 400, invalid style 400, success shape with styledContent/changes/styleName, all 7 valid styles accepted, AI returns null → 500, AI error → 500 — 11 tests)

### Phase 260: AI-Powered Engagement CTA & Hook Generator
- [x] Add `generateEngagementCTAs(content, platforms, ctaType?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate 5 platform-optimised engagement hooks and calls-to-action for the given post content; ctaType options: `"question"`, `"challenge"`, `"poll"`, `"share"`, `"comment"`, `"save"`, `"follow"`, `"link"`, `"general"`; uses prompt caching on system block; returns `{ctas: {text: string, type: string, platform?: string, engagementBoost: "low"|"medium"|"high", explanation: string}[], hook: string}` or null when AI not configured
- [x] `POST /api/ai/engagement-ctas` endpoint — auth + rate limit + zod validation (`content: string min 10 max 10000`, `platforms: string[] min 1`, `ctaType?: string`); calls `generateEngagementCTAs`; returns `{ctas, hook}`; 503 when AI not configured; 500 when AI returns null
- [x] `EngagementCTADialog` component (`src/components/engagement-cta-dialog.tsx`) — modal with optional CTA type filter chips (Question / Challenge / Poll / Share / Comment / Save / General); shows 5 generated CTAs with engagement boost badges (Low/Medium/High coloured pills), explanation text, "Append to Post" button that adds selected CTA to end of content, copy-to-clipboard per CTA; separate "Best Hook" section showing AI-suggested opening line with "Prepend to Post" button; regenerate button; loading state
- [x] "CTA" button (Target icon) in post composer toolbar — opens `EngagementCTADialog` with current content and selected platforms; disabled when content is empty; `onAppendCTA` appends CTA text to content; `onPrependHook` prepends hook as opening line
- [x] Unit tests for `POST /api/ai/engagement-ctas` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, content too short 400, empty platforms 400, success shape with ctas array and hook string, ctaType param forwarded, cta shape with text/type/engagementBoost/explanation, engagementBoost valid values, AI returns null → 500, AI error → 500 — 12 tests)

### Phase 261: AI-Powered Legal & Regulatory Compliance Checker
- [x] Add `checkLegalCompliance(content, industry, platforms, country?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to analyze post content for legal and regulatory compliance issues; checks for FTC disclosure requirements, medical/health claims, financial advice disclaimers, contest rules, competitor disparagement, age-restriction disclosures, greenwashing claims, IP/copyright risks, and platform policy violations; uses prompt caching on system block; returns `{compliant: boolean, issues: {type, severity: "low"|"medium"|"high", regulation, description, suggestion}[], overallRisk: "low"|"medium"|"high", summary: string}` or null when AI not configured
- [x] `POST /api/ai/legal-compliance` endpoint — auth + rate limit + zod validation (`content: string min 10 max 10000`, `platforms: string[] min 1`, `industry: enum (general|healthcare|finance|food_beverage|beauty_cosmetics|fitness_wellness|legal_services|real_estate|education|retail_ecommerce|travel_hospitality|technology|entertainment|non_profit) default "general"`, `country?: string`); calls `checkLegalCompliance`; returns compliance result; returns 503 when AI not configured; returns 500 when AI returns null
- [x] `LegalComplianceDialog` component (`src/components/legal-compliance-dialog.tsx`) — modal with industry selector (14 industry options) and country/region dropdown (US/UK/EU/CA/AU/SG/AE); "Check Compliance" button calls endpoint; results show risk badge (Low/Medium/High), overall summary, issue count breakdown by severity, per-issue expandable cards with type, regulation cited, description, and fix suggestion; issues sorted high→medium→low severity; loading state; re-check option
- [x] "Legal" button (Scale icon) in post composer toolbar — opens `LegalComplianceDialog` with current content and selected platforms; disabled when content is empty
- [x] Unit tests for `POST /api/ai/legal-compliance` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, content too short 400, empty platforms 400, invalid industry 400, compliant result shape, flagged result with issues, issue shape fields, industry and country forwarded, default industry "general", AI returns null → 500, AI error → 500 — 14 tests)

### Phase 262: AI Writing Coach
- [x] Add `WritingCoachFeedback`, `WritingCoachImprovement`, `WritingCoachCategory`, `WritingCoachImpact` types + `getWritingCoachFeedback(content, platforms, tone?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to analyze post content and return a 0–100 score, summary, and up to 5 improvement suggestions across categories (clarity/engagement/platform/tone/cta) with impact levels (high/medium/low); uses prompt caching on system block; returns null when AI not configured or parse fails
- [x] `POST /api/ai/writing-coach` endpoint — auth + rate limit + zod validation (`content: string min 20 max 10000`, `platforms: string[] min 1`, `tone?: string`); checks ANTHROPIC_API_KEY; calls `getWritingCoachFeedback`; returns feedback object; returns 401/429/503/400/500 on respective error conditions
- [x] `WritingCoachPanel` component (`src/components/writing-coach-panel.tsx`) — toggleable panel (starts as a compact "Writing Coach" button, expands to full feedback UI); 1500ms debounce on content/platform/tone changes; shows score badge (green ≥70/yellow ≥40/red <40), improvement list with category icons and impact colour chips; refresh and collapse controls; suppresses duplicate fetches via `lastFetchRef`
- [x] Integrate `WritingCoachPanel` into post composer below `ContentModerationBadge` and above `PerformancePredictionCard`
- [x] Unit tests for `POST /api/ai/writing-coach` (auth 401, rate limit 429, no API key 503, content too short 400, empty platforms 400, invalid JSON 400, success shape, improvement fields, valid categories, valid impacts, AI returns null 500, AI error 500 — 12 tests)

### Phase 263: AI-Powered Social Media Image Prompt Generator
- [x] Add `ImagePromptResult` interface + `generateImagePrompts(content, platforms, style?, mood?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate platform-optimized image creation prompts; defines per-platform aspect ratios (Instagram 1:1/4:5, Facebook 1.91:1, Twitter 16:9, LinkedIn 1.91:1, Pinterest 2:3, TikTok 9:16, Threads 1:1, etc.); uses prompt caching on system block; returns `ImagePromptResult[]` where each item has `{platform, prompt, negativePrompt, aspectRatio, style, keyElements: string[], colorPalette: string[], mood}`; returns empty array on parse failure
- [x] `POST /api/ai/image-prompts` endpoint — auth + rate limit + zod validation (`content: string min 10 max 10000`, `platforms: string[] min 1`, `style?: string max 50`, `mood?: string max 50`); calls `generateImagePrompts`; returns `{prompts}`; 503 when AI not configured; 500 when prompts array is empty
- [x] `ImagePromptsDialog` component (`src/components/image-prompts-dialog.tsx`) — modal with image style selector (Photo/Illustration/Graphic/Infographic/Abstract) and mood selector (Professional/Playful/Minimalist/Vibrant/Dramatic/Warm/Dark); "Generate" / "Regenerate" button; per-platform result cards showing: aspect ratio badge, style badge, main prompt in monospace box, negative prompt in red-tinted box, key elements chips, color palette swatches with hex codes; copy-to-clipboard per prompt; loading/empty states
- [x] "Image" button (ImageIcon) in post composer toolbar — opens `ImagePromptsDialog` with current content and selected platforms; disabled when content is empty
- [x] Unit tests for `POST /api/ai/image-prompts` (auth 401, rate limit 429, AI disabled 503, content too short 400, empty platforms 400, invalid JSON 400, success shape with prompts array, prompt fields present, style and mood forwarded to AI, AI returns empty → 500, AI error → 500 — 10 tests)

### Phase 264: Publishing Streak Tracker & Gamification
- [x] `GET /api/analytics/publishing-streak` endpoint — auth + rate limit; queries PUBLISHED PublishResults; computes `currentStreak` (consecutive calendar days ending today/yesterday with ≥1 published post), `longestStreak` (all-time best), `streakStartDate` (ISO date or null), `totalActiveDays` (distinct days with activity), `last30Days` (array of 30 `{date, count}` objects newest-first), `streakLabel` ("🔥 X-day streak" or "No current streak"); returns full streak object
- [x] `PublishingStreakCard` component (`src/components/publishing-streak-card.tsx`) — shows flame emoji + current streak count, longest streak badge, total active days, 30-day activity dot grid (green = published, gray = no activity), "Start your streak!" empty state, loading skeleton
- [x] Add `"publishing_streak"` to `WIDGET_KEYS` and `publishing_streak: "Publishing Streak"` to `WIDGET_LABELS` in dashboard-widgets route; integrate `PublishingStreakCard` into analytics dashboard below `TagPerformanceCard`
- [x] Unit tests for publishing-streak endpoint (auth 401, rate limit 429, empty state zeros, currentStreak consecutive days, longestStreak all-time best, last30Days 30 entries, entry shape {date, count}, streakStartDate null when 0, streakLabel with count, DB error 500 — 10 tests)

### Phase 265: AI-Generated Carousel & Multi-Slide Content Planner
- [x] Add `generateCarouselContent(topic, slideCount, platforms, tone?, audience?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate structured carousel/slide-deck content for social platforms; exports `CarouselSlide` and `CarouselContent` interfaces; uses prompt caching on system block; returns `{title, coverSlide: {headline, subtitle}, slides: [{slideNumber, headline, bodyText, visualDescription, keyTakeaway}], closingSlide: {cta, hashtags[]}}` or null when AI not configured or parse fails
- [x] `POST /api/ai/carousel-content` endpoint — auth + rate limit + zod validation (`topic: string min 2 max 300`, `slideCount: 3–15 default 5`, `platforms: string[] min 1`, `tone?: string max 100`, `audience?: string max 200`); calls `generateCarouselContent`; returns `{carousel}`; 503 when AI not configured; 500 when AI returns null
- [x] `CarouselContentDialog` component (`src/components/carousel-content-dialog.tsx`) — modal with topic input, slide count range slider (3–15), tone selector, audience input; calls `/api/ai/carousel-content`; displays cover slide, numbered content slides (headline, body, key takeaway, visual description), and closing slide with CTA + hashtags; per-slide copy button; "Copy Full Carousel" button builds text summary of all slides; "Use in Composer" applies text to post content; regenerate option; loading state
- [x] "Carousel" button (`GalleryHorizontal` icon) in post composer toolbar — opens `CarouselContentDialog` with currently selected platforms; `onApply` callback sets post content
- [x] Unit tests for `POST /api/ai/carousel-content` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, topic missing 400, topic too short 400, empty platforms 400, slideCount below minimum 400, slideCount above maximum 400, success with carousel shape, carousel has coverSlide/slides/closingSlide, tone and audience forwarded, slideCount defaults to 5, AI returns null → 500, AI error → 500 — 15 tests)

### Phase 266: Evergreen Content Discovery Engine & Recycling Recommendations
- [x] Evergreen score utility (`src/lib/evergreen-score.ts`) — `computeEvergreenScore(post, insights)` scores a PUBLISHED post 0–100 for recycling potential based on: engagement rate vs user average (40%), content age without degradation (20%), timeless topic keywords (no date/news/breaking/trend references, 30%), hashtag diversity (10%); exports `EvergreenCandidate` interface and `evergreenLabel(score): "Excellent"|"Good"|"Fair"|"Poor"`
- [x] `GET /api/analytics/evergreen-candidates` endpoint — auth + rate limit + `?limit=20&minScore=40`; queries user's PUBLISHED posts (isEvergreen=false) with PostInsights; computes evergreen scores; returns `{candidates: EvergreenCandidate[], totalAnalyzed, avgScore}` sorted by score desc; only includes posts with at least one PostInsights record
- [x] `EvergreenDiscoveryCard` component (`src/components/evergreen-discovery-card.tsx`) — ranked list of top recycling candidates; each card shows score badge (green ≥70/yellow ≥50/orange ≥30/red <30), content preview (60 chars), engagement metrics (likes/comments/shares), post age in days, "Mark Evergreen" button (calls `PATCH /api/posts/[id]/evergreen`), "Recycle Now" button (calls `POST /api/posts/[id]/recycle`); period selector (7d/30d/90d); loading skeleton; "No published posts yet" empty state
- [x] Add "Discover" button to the posts page header — opens a modal containing `EvergreenDiscoveryCard` when clicked; modal refreshes after marking or recycling a post
- [x] Unit tests for evergreen score utility (score 0–100, high engagement = high score, all time-sensitive keywords lower score, old-but-engaging = high, hashtag bonus, `evergreenLabel` boundaries — 8 tests) and `GET /api/analytics/evergreen-candidates` endpoint (auth 401, rate limit 429, limit param, minScore filter, already-evergreen excluded, no-insights excluded, sorted by score desc, response shape — 10 tests)

### Phase 267: Content Health Dashboard
- [x] Content health utility (`src/lib/content-health.ts`) — `computeContentHealth(input: ContentHealthInput)` scores holistic content health across 5 weighted dimensions summing to 100: Content Diversity (30pts, full marks at ≥5 categories), Platform Coverage (20pts, proportional to accounts posted to), Posting Regularity (20pts, maps from consistency score), Engagement Trend (20pts, compares current vs prior period), Content Freshness (10pts, penalises >20% recycled posts); returns `{overallScore, overallLabel, dimensions, recommendations}`; labels: Excellent ≥85, Good ≥65, Fair ≥40, Needs Attention <40
- [x] `GET /api/analytics/content-health` endpoint — auth + rate limit + `?period=30d|90d` (default 30d); queries posts for current and prior periods, active social account count, distinct accounts posted to, recycled post count; computes consistency via `computeConsistency`; calls `computeContentHealth`; returns score + dimensions + recommendations + `period`
- [x] `ContentHealthCard` component (`src/components/content-health-card.tsx`) — SVG score ring (colour-coded green/blue/amber/red by score), dimension progress bars with labels, recommendations panel, period selector (30d/90d), Refresh button; integrated into analytics dashboard; `"content_health"` widget key added to `WIDGET_KEYS` and `WIDGET_LABELS`
- [x] Unit tests for content-health utility (15 tests — score bounds, 5 dimensions, full diversity at 5 categories, category recommendation, full coverage, partial coverage recommendation, consistency mapping, trend above-neutral, engagement trend neutral when no prior, full freshness, reduced freshness, Excellent label, Needs Attention label)
- [x] Unit tests for `GET /api/analytics/content-health` API (10 tests — 401 unauth, 429 rate limit, 400 invalid period, 200 valid shape, 5 dimensions, period echoed, default 30d, score bounds, dimension field shapes, non-empty recommendations, higher diversity with 5 categories)

### Phase 268: AI-Generated Newsletter from Social Posts
- [x] Add `generateNewsletterDraft(posts, periodLabel, tone?, customIntro?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to compile user's recent published posts into a cohesive newsletter format; uses prompt caching on system block; returns `{subject: string, intro: string, sections: {headline: string, excerpt: string, platform: string, content: string}[], keyTakeaways: string[], conclusion: string, estimatedReadTime: number}` or null when AI not configured
- [x] `POST /api/ai/newsletter-draft` endpoint — auth + rate limit + zod validation (`period: "week"|"month"|"custom"`, `from?: ISO date string`, `to?: ISO date string`, `tone?: string`, `intro?: string max 500 chars`); when period="custom" both `from` and `to` are required; fetches user's PUBLISHED posts in the period (most recent 20); calls `generateNewsletterDraft`; returns `{newsletter, postCount}`; returns 503 when AI not configured; returns 500 when AI returns null
- [x] `NewsletterDraftDialog` component (`src/components/newsletter-draft-dialog.tsx`) — modal with period selector (This Week / This Month / Custom date range), optional custom intro textarea, optional tone selector (professional/casual/educational/inspiring/entertaining); calls `/api/ai/newsletter-draft`; shows: subject line with copy button, intro text, per-section cards (headline + excerpt for each post), key takeaways list, conclusion; "Copy Full Newsletter" button exports complete formatted text; "Regenerate" button; loading state; post count indicator showing how many posts will be included
- [x] Newsletter draft page in dashboard (`/newsletter`) — shows `NewsletterDraftDialog` inline as the main content; left panel with period/options form and post count preview; right panel with generated newsletter; "Export as Text" download button; Add "Newsletter" to sidebar navigation (icon: `Newspaper`) before "Settings"
- [x] Unit tests for `POST /api/ai/newsletter-draft` (auth 401, rate limit 429, AI disabled 503, missing period 400, invalid period 400, custom period missing from/to 400, invalid date format 400, success shape, newsletter fields present, sections array, key takeaways array, post count returned, AI returns null 500, AI error 500 — 13 tests)

### Phase 269: Scheduling Analytics & Calendar Intelligence
- [x] `GET /api/analytics/scheduling-analytics` endpoint — auth + rate limit + `?period=30d|90d` (default 30d); queries user's SCHEDULED and PUBLISHED posts within the period; computes: `occupancyRate` (% of days with ≥1 post, 0–100), `avgPostsPerActiveDay` (avg posts on non-empty days), `dayDistribution` (`{dayName, count}[]` Sun–Sat by UTC day-of-week), `hourDistribution` (`{hour, count}[]` for all 24 hours), `avgLeadTimeDays` (median days between createdAt and scheduledAt for SCHEDULED posts, null if no data), `platformBalance` (`{platform, count}[]` from publishResults sorted by count desc), `totalScheduled`, `totalPublished`; returns 400 for invalid period
- [x] `SchedulingAnalyticsCard` component (`src/components/scheduling-analytics-card.tsx`) — client component; period selector (30d/90d); shows calendar occupancy progress bar (green ≥70% / yellow ≥40% / red <40%), summary stats row (total posts / posts per active day / median lead time), day-of-week bar chart (7 columns), hour-of-day bar chart (24 columns), platform balance rows with percentage bars; loading skeleton; "No scheduling data yet" empty state
- [x] Integrate `SchedulingAnalyticsCard` into analytics dashboard (`src/components/analytics-dashboard.tsx`) below `ContentHealthCard`; add `"scheduling_analytics"` to `WIDGET_KEYS` array and `scheduling_analytics: "Scheduling Analytics"` to `WIDGET_LABELS` in `src/app/api/dashboard-widgets/route.ts`; wrap with `isVisible("scheduling_analytics")` guard
- [x] Unit tests for the endpoint (auth 401, rate limit 429, invalid period 400, empty state zeros, period echoed, defaults to 30d, dayDistribution 7 entries with Sun–Sat names, hourDistribution 24 entries, occupancyRate 0–100 bounds, correct response shape — 10 tests)

### Phase 270: AI-Powered Social Media Ad Copy Generator
- [x] Add `AdCopyVariant` and `AdCopyResult` interfaces + `generateAdCopy(content, platforms, objective, targetAudience?, budget?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` with platform-specific headline/primary text character limits; supports 7 campaign objectives (awareness/traffic/engagement/leads/sales/app_installs/general) and 3 budget tiers (small/medium/large); uses prompt caching on system block; returns `{variants: AdCopyVariant[], guidelines: string[]}`
- [x] `POST /api/ai/ad-copy` endpoint — auth + rate limit + AI check + zod validation (`content: string min 10`, `platforms: string[] min 1`, `objective: enum default "general"`, optional `targetAudience: string max 300`, optional `budget: enum`); calls `generateAdCopy`; returns ad copy result; 503 when AI not configured
- [x] `AdCopyDialog` component (`src/components/ad-copy-dialog.tsx`) — modal with campaign objective selector (7 options), budget level selector, target audience input; shows per-platform variant cards (headline with char count, primary text with char count, CTA badge, targeting notes, copy headline/text and "Use in Post" buttons); char counts color-coded (red over limit, yellow at 90%)
- [x] "Ad Copy" button (Megaphone icon) in post composer toolbar — opens `AdCopyDialog` with current content and selected platforms; `onApply` sets post content to selected primary text
- [x] Unit tests for ad-copy endpoint (auth, rate limit, AI disabled, invalid JSON, content too short, empty platforms, success shape, objective/audience forwarding, budget forwarding, default objective "general", null result → 500, unexpected error → 500 — 12 tests)

### Phase 271: Workflow Efficiency Analytics & Time-to-Publish Tracking
- [x] `GET /api/analytics/workflow-efficiency` endpoint — auth + rate limit + `?period=30d|90d` (default 30d); queries user's posts by status and their associated publishResults; computes: `avgDraftToScheduledHours` (avg hours from post createdAt to scheduledAt for SCHEDULED/PUBLISHED posts with a scheduledAt in period), `avgScheduledToPublishedHours` (avg hours from scheduledAt to first publishedAt for PUBLISHED results in period), `avgDraftToPublishedHours` (avg total hours from post createdAt to first publishedAt), `fastestPublishHours` (minimum total draft-to-published hours, null when no data), `slowestPublishHours` (maximum, null when no data), `postsPublished` (count of PUBLISHED posts in period), `postsStillDraft` (count of DRAFT posts updated in period), `statusDistribution: {status: string, count: number}[]`; returns 400 for invalid period
- [x] `WorkflowEfficiencyCard` component (`src/components/workflow-efficiency-card.tsx`) — "use client"; period selector (30d/90d); time funnel showing draft→scheduled→published avg hours as color-coded stat boxes; fastest/slowest publish time chips; status distribution bar chart; loading skeleton with animate-pulse; "No published posts yet" empty state
- [x] Add `"workflow_efficiency"` to `WIDGET_KEYS` and `workflow_efficiency: "Workflow Efficiency Analytics"` to `WIDGET_LABELS` in dashboard-widgets route; integrate `WorkflowEfficiencyCard` into analytics dashboard below `SchedulingAnalyticsCard` with `isVisible("workflow_efficiency")` guard
- [x] Unit tests for workflow-efficiency endpoint (auth 401, rate limit 429, invalid period 400, empty state shape, period echoed, defaults to 30d, avgDraftToScheduledHours calculation, response shape, postsStillDraft count, DB error 500 — 10 tests)

### Phase 272: Engagement Funnel Analytics & Conversion Rate Tracking
- [x] `GET /api/analytics/engagement-funnel` endpoint — auth + rate limit + `?period=30d|90d`; queries user's PUBLISHED PublishResult records with PostInsights; aggregates per-platform impressions, reach, and engagement (likes+comments+shares); computes reachRate (reach/impressions×100), engagementRate (engagement/impressions×100), engagementFromReachRate (engagement/reach×100); sorts platforms by impressions desc; identifies topPlatform by highest engagementRate; returns `{period, overall: {impressions, reach, engagement, reachRate, engagementRate, engagementFromReachRate}, platforms: FunnelPlatformData[], topPlatform}`
- [x] `EngagementFunnelCard` component (`src/components/engagement-funnel-card.tsx`) — funnel visualization with proportional horizontal bars (Impressions=indigo, Reach=blue, Engagement=violet), three KPI boxes (Reach Rate, Engagement Rate, Engage/Reach), per-platform breakdown table sorted by impressions, topPlatform callout, period selector (30d/90d), loading skeleton, "No insights data yet" empty state
- [x] Integrate `EngagementFunnelCard` into analytics dashboard below `WorkflowEfficiencyCard`; add `"engagement_funnel"` widget key and label `"Engagement Funnel Analytics"` to `WIDGET_KEYS` and `WIDGET_LABELS` in dashboard-widgets route; add to `DashboardCustomizeDialog`
- [x] Unit tests for `GET /api/analytics/engagement-funnel` (auth, rate limit, invalid period, empty shape with null rates, default period, period echo, impressions/reach/engagement aggregation, reachRate/engagementRate/engagementFromReachRate calculation, platforms sorted by impressions desc, topPlatform by engagement rate, multiple insights per result, 500 on DB error — 12 tests)

### Phase 273: Social Media Growth Velocity Dashboard & Momentum Tracking
- [x] `GET /api/analytics/growth-velocity` endpoint — auth + rate limit + `?period=30d|90d|180d` (default 30d); queries active SocialAccounts with AudienceMetric records in period; computes followerVelocityPerDay, followerAcceleration (second-half vs first-half velocity), engagementVelocityPerPost, momentumScore (0–100: velocityScore+accelScore+engScore), momentumLabel ("Rising"/"Stable"/"Declining"/"Insufficient Data"); returns `{period, periodDays, accounts: AccountVelocityData[], fleetMomentumScore, topMomentumAccount}`
- [x] `GrowthVelocityCard` component (`src/components/growth-velocity-card.tsx`) — "use client"; period selector (30d/90d/180d); fleet summary banner (fleetMomentumScore/100, topMomentumAccount); Recharts LineChart of daily follower counts per account; per-account breakdown with MomentumBadge (Rising/Stable/Declining/Insufficient Data), currentFollowers, followerGainTotal, followerVelocityPerDay, followerAcceleration; loading/error/empty states
- [x] Integrate `GrowthVelocityCard` into analytics dashboard below `EngagementFunnelCard`; add `"growth_velocity"` widget key and label `"Growth Velocity Dashboard"` to `WIDGET_KEYS` and `WIDGET_LABELS` in dashboard-widgets route
- [x] Unit tests for growth-velocity endpoint (auth 401, rate limit 429, invalid period 400, empty shape, period echoed, velocity from two metrics, Insufficient Data with one metric, positive acceleration, Rising label, fleetMomentumScore average, topMomentumAccount selection, DB error 500 — 12 tests)

### Phase 274: Engagement Depth & Interaction Quality Analytics
- [x] `GET /api/analytics/engagement-depth` endpoint — auth + rate limit + `?period=7d|30d|90d|all` (default 30d); queries user's PUBLISHED posts with publishResults and PostInsights; filters zero-impression rows; computes per-row commentRate (comments/impressions×100) and shareRate (shares/impressions×100); aggregates per-platform avgCommentRate, avgShareRate, postCount; returns top 10 posts sorted by commentRate; returns `{period, avgCommentRate, avgShareRate, avgEngagementDepthScore, platformMetrics, topDeepEngagementPosts, totalAnalyzed}`
- [x] `EngagementDepthCard` component (`src/components/engagement-depth-card.tsx`) — "use client"; period selector (7d/30d/90d/all); `DepthScoreBadge` sub-component (green ≥70/amber ≥40/red <40); summary metrics row (avgCommentRate%, avgShareRate%, depth score/100, post count); platform breakdown table with color-coded rates; top 5 conversation-driving posts; loading skeleton, error state, "No engagement data yet" empty state
- [x] Integrate `EngagementDepthCard` into analytics dashboard below `GrowthVelocityCard`; add `"engagement_depth"` widget key and label `"Engagement Depth & Interaction Quality"` to `WIDGET_KEYS` and `WIDGET_LABELS` in dashboard-widgets route; add to `DashboardCustomizeDialog`
- [x] Unit tests for engagement-depth endpoint (auth, rate limit, invalid period, empty state shape with zeros and empty arrays, avgCommentRate calculation, avgShareRate calculation, depthScore in 0-100 range, platformMetrics shape, topDeepEngagementPosts capped at 10, zero-impression rows skipped, "all" period returns 200, DB error 500 — 11 tests)

### Phase 275: AI-Powered Platform Selector & Distribution Optimizer
- [x] Add `suggestOptimalPlatforms(content, mediaType, userPlatforms, historicalContext?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` with prompt caching on system block; analyzes post content/mediaType and recommends which of the user's connected platforms would perform best; returns `{suggestions: {platform: string, score: number, reasoning: string, bestForAudience: string, tips: string[]}[], overallStrategy: string}` or null when AI not configured
- [x] `POST /api/ai/suggest-platforms` endpoint — auth + rate limit + zod validation (`content: string min 10 max 10000`, `mediaType: "NONE"|"IMAGE"|"VIDEO"|"CAROUSEL"`, `platforms: string[] min 1`); optionally queries user's PostInsights to build historical context summary; calls `suggestOptimalPlatforms`; returns `{suggestions, overallStrategy}`; 503 when AI not configured; 500 when AI returns null
- [x] `PlatformSuggestionDialog` component (`src/components/platform-suggestion-dialog.tsx`) — modal showing AI-ranked platforms with score bar (0–100, green ≥70/yellow ≥40/red <40), reasoning paragraph, bestForAudience label, and tips list; "Select Recommended" button auto-checks suggested platforms; regenerate button; loading state
- [x] "Suggest Platforms" button (Sparkles icon) in post composer account selector section — opens `PlatformSuggestionDialog` with current content, mediaType, and connected platforms; `onSuggest(platforms: string[])` callback selects the recommended accounts in the composer
- [x] Unit tests for `POST /api/ai/suggest-platforms` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, content too short 400, empty platforms 400, success shape with suggestions/overallStrategy, suggestion fields present, overallStrategy string, AI returns null → 500, AI error → 500 — 11 tests)

### Phase 276: Content Queue Health Monitor & Publishing Runway Tracker
- [x] Queue health utility (`src/lib/queue-health.ts`) — `computeQueueHealth(scheduledPosts)` computes scheduledCount, avgPostsPerDay (posts in next 30 days / 30), queueRunwayDays (days until last scheduled post within 30-day window), queueStatus ("healthy" ≥14d / "low" 7–13d / "critical" <7d / "empty"), contentGapDays (next 14 calendar days with no scheduled posts), nextScheduledAt (ISO string of soonest future post), platformBreakdown (per-platform post count sorted by count desc); exports `ScheduledPost`, `QueueHealthResult`, `QueueStatus`, `ContentGap` types
- [x] `GET /api/analytics/queue-health` endpoint — auth + rate limit; fetches user's SCHEDULED posts with publishResults via Prisma; filters null scheduledAt; maps to `ScheduledPost[]`; calls `computeQueueHealth`; returns `QueueHealthResponse`; handles DB errors with 500
- [x] `QueueHealthCard` component (`src/components/queue-health-card.tsx`) — "use client"; fetches `/api/analytics/queue-health` on mount; shows status badge (green/yellow/red/grey with emoji), KPI row (scheduled count, avg/day, runway days), next post relative time, platform breakdown bar chips, content gap pills (next 14 days), loading skeleton; integrated into analytics dashboard with `isVisible("queue_health")` guard
- [x] Add `"queue_health"` to `WIDGET_KEYS` array and `queue_health: "Publishing Queue Health"` to `WIDGET_LABELS` in `src/app/api/dashboard-widgets/route.ts`
- [x] Unit tests for queue health utility (11 tests — empty state, 14 content gaps, healthy at ≥14d runway, low at 7–13d, critical at <7d, avgPostsPerDay from 30-day window, past posts excluded from calculations, content gap detection, platform breakdown sorted desc, nextScheduledAt soonest post, runway from posts beyond 30d) and API route (9 tests — auth, rate limit, empty state, scheduled count, platform breakdown, null scheduledAt filtering, DB error, userId filter, nextScheduledAt)

### Phase 277: Post Performance Anomaly Detection & Alert System
- [x] Anomaly detection utility (`src/lib/anomaly-detection.ts`) — `computeWeightedEngagement(likes, comments, shares, reach, impressions)` applies weighted formula (likes×3 + comments×5 + shares×4 + reach×1 + impressions×0.5); `computeZScore(value, mean, stddev)` returns 0 when stddev is 0; `detectEngagementAnomalies(posts, zThreshold=2.0)` groups posts by platform, computes population mean+stddev per platform (requires ≥3 samples), flags posts with |z| ≥ zThreshold as spike (z≥threshold) or drop (z≤-threshold); returns anomalies sorted by absolute z-score desc and platformBaselines sorted by sampleSize desc; exports `PostForAnomaly`, `PostAnomaly`, `AnomalyType`, `AnomalyDetectionResult` interfaces
- [x] `GET /api/analytics/anomalies` endpoint — auth + rate limit + `?period=7d|30d|90d|all` (default 30d, returns 400 for invalid); queries user's PUBLISHED posts with publishResults (PUBLISHED status) and PostInsights; flattens to one PostForAnomaly per (post × platform × insight); calls `detectEngagementAnomalies`; returns `{period, anomalies, totalAnalyzed, platformBaselines}`
- [x] `AnomalyDetectionCard` component (`src/components/anomaly-detection-card.tsx`) — "use client"; period selector (7d/30d/90d/all); Refresh button; loading skeleton; empty state when no anomalies; summary chips (spike count + drop count); anomaly list (up to 10) with type badge, platform, z-score, engagement vs mean; baselines section showing per-platform mean/stddev/sampleSize
- [x] Add `"anomalies"` to `WIDGET_KEYS` array and `anomalies: "Performance Anomaly Detection"` to `WIDGET_LABELS` in `src/app/api/dashboard-widgets/route.ts`; integrate `AnomalyDetectionCard` into analytics dashboard below `QueueHealthCard` with `isVisible("anomalies")` guard
- [x] Unit tests for anomaly detection utility and API endpoint (computeWeightedEngagement weights, computeZScore formula, detectEngagementAnomalies empty/insufficient/spike/drop/no-flag/sort-order/totalAnalyzed/platformBaselines/null-insights/custom-threshold — 17 tests)

### Phase 278: Content Accessibility & Inclusive Language Compliance
- [x] Add `checkContentAccessibility(content, altTexts?, platform?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to audit post content for accessibility and inclusivity: checks readability complexity (jargon/complex words), emoji screen reader compatibility (warns when emoji-heavy mid-sentence), hashtag casing (recommends CamelCase for screen readers), non-inclusive/ableist language patterns, missing alt text when media referenced, and overly long sentences; uses prompt caching on system block; returns `{score: number (0–100), passesStandards: boolean, issues: {type: "readability"|"inclusive_language"|"emoji"|"hashtag_casing"|"alt_text"|"sentence_length", severity: "low"|"medium"|"high", text?: string, suggestion: string, explanation: string}[], recommendations: string[], summary: string}` or null when AI not configured
- [x] `POST /api/ai/accessibility-check` endpoint — auth + rate limit + zod validation (`content: string min 1 max 10000`, `altTexts?: string[] max 10 items`, `platform?: string`); calls `checkContentAccessibility`; returns accessibility result; returns 503 when AI not configured; returns 500 when AI returns null
- [x] `AccessibilityCheckerPanel` component (`src/components/accessibility-checker-panel.tsx`) — "use client" collapsible panel triggered by "Accessibility" button below post composer; shows score badge (green ≥80/yellow ≥60/red <60), `passesStandards` boolean indicator, issue list grouped by severity with expandable explanations and suggestions, overall recommendations list; 1200ms debounce on content changes; loading spinner while checking; "No issues found" empty state; passes current `altTexts` from the post composer
- [x] Unit tests for `POST /api/ai/accessibility-check` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, content missing 400, content too long 400, success shape with score/passesStandards/issues/recommendations, altTexts forwarded, platform forwarded, AI returns null → 500, AI error → 500 — 11 tests)

### Phase 279: AI-Powered Content Summarizer & Key Points Extractor
- [x] Add `summarizeContent(content, targetLength, style?, platforms?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to intelligently summarize post content; supports `style` options ("bullet_points"|"narrative"|"headline"); `targetLength` in characters (50–2200); preserves hashtags and key mentions; uses prompt caching on system block; returns `{summary: string, keyPoints: string[], title?: string, charCount: number}` or null when AI not configured
- [x] `POST /api/ai/summarize` endpoint — auth + rate limit + zod validation (`content: string min 50 max 50000`, `targetLength?: number 50–2200 default 280`, `style?: "bullet_points"|"narrative"|"headline"`, `platforms?: string[]`); calls `summarizeContent`; returns `{summary, keyPoints, title, charCount}`; 503 when AI not configured; 500 when AI returns null
- [x] `ContentSummarizerDialog` component (`src/components/content-summarizer-dialog.tsx`) — modal with target length slider (50–2200 chars, snapping to platform presets: Twitter 280/Instagram 2200/LinkedIn 700/Threads 500), style selector (Narrative/Bullet Points/Headline Only), optional platform context chips; shows summarized output with char count vs limit color indicator, key points list, copy-to-clipboard, "Use in Composer" button; regenerate option; loading state
- [x] "Summarize" button (`Scissors` icon) in post composer toolbar — opens `ContentSummarizerDialog`; disabled when content has fewer than 50 chars; `onApply` sets post content to the summary
- [x] Unit tests for `POST /api/ai/summarize` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, content too short 400, content too long 400, targetLength out of range 400, success shape with summary/keyPoints/title/charCount, style param forwarded, platforms forwarded, AI returns null → 500, AI error → 500 — 12 tests)

### Phase 280: AI-Powered Social Media Account Growth Strategy Generator
- [x] Add `generateGrowthStrategy(metrics, platforms, goals?, timeframe?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` with user's current account performance data (follower counts, engagement rates, posting frequency, top content categories) and generates a personalized growth strategy; uses prompt caching on system block; returns `{weeks: {week: number, focus: string, tactics: string[], kpis: string[]}[], platformSpecific: {platform: string, tips: string[]}[], overallApproach: string, estimatedGrowth: string}` or null when AI not configured
- [x] `POST /api/ai/growth-strategy` endpoint — auth + rate limit + AI check + zod validation (`platforms: string[] min 1`, `goals?: string max 500`, `timeframe?: "30d"|"90d" default "30d"`); queries user's latest AudienceMetrics, PostInsights (30d), and active SocialAccounts; calls `generateGrowthStrategy`; returns `{strategy, generatedAt}`; 503 when AI not configured
- [x] `GrowthStrategyDialog` component (`src/components/growth-strategy-dialog.tsx`) — modal with goals input (max 500 chars), timeframe selector (30 Days / 90 Days), connected platform chips pre-selected; calls `/api/ai/growth-strategy`; shows weekly breakdown accordion, platform-specific tips section, overall approach card, estimated growth callout; regenerate button; loading/empty states
- [x] "Growth Strategy" button (`TrendingUp` icon) in audience page header (`/audience`) — client component that opens `GrowthStrategyDialog` with all connected platform names pre-passed
- [x] Unit tests for `POST /api/ai/growth-strategy` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, empty platforms 400, success shape with strategy/generatedAt, strategy has weeks array, platformSpecific tips included, AI error 500 — 9 tests)

### Phase 281: Content Template Marketplace & Community Library
- [x] Add `marketplacePublished` (Boolean default false) + `marketplaceCategory` (nullable String) + `marketplaceTags` (String[] default []) + `importCount` (Int default 0) to Template model + Prisma migration (`20261002000000_add_template_marketplace`)
- [x] `TemplateImport` model in Prisma (id, userId, templateId, importedAt) + `@@unique([userId, templateId])` — tracks which templates each user has imported; prevents duplicate imports and drives importCount
- [x] `GET /api/marketplace/templates` endpoint — no auth required; returns published templates sorted by importCount desc; supports `?category=&tag=&search=&limit=20&page=` query params; strips user PII (returns templateId, name, content preview truncated 150 chars, mediaType, category, tags, importCount, createdAt only)
- [x] `POST /api/templates/[id]/publish` endpoint — auth + rate limit + ownership check; validates template has non-empty content; accepts optional `{category?: string, tags?: string[]}` body; sets marketplacePublished=true; returns `{marketplacePublished, marketplaceCategory, marketplaceTags}`
- [x] `POST /api/templates/[id]/unpublish` endpoint — auth + rate limit + ownership check; sets marketplacePublished=false; returns `{marketplacePublished: false}`
- [x] `POST /api/marketplace/templates/[id]/import` endpoint — auth + rate limit; finds published template by id; upserts TemplateImport row (idempotent); when newly created, copies template content to user's own Template record and increments importCount; returns `{templateId, alreadyImported, newTemplateId?}`
...
### Phase 283: Smart Calendar Gap Filler & Auto-Publishing Assistant
- [x] Calendar gap detection utility (`src/lib/calendar-gaps.ts`) — `detectCalendarGaps(scheduledPosts, windowDays)` builds set of covered YYYY-MM-DD dates, iterates tomorrow UTC through tomorrow+windowDays, returns `CalendarGapResult` with gaps[], totalGaps, coveredDays, analyzedDays, gapRate (0–100), longestStreakDays
- [x] `GET /api/analytics/calendar-gaps` endpoint — auth + rate limit (single-arg apiLimiter) + `?windowDays=1-60` (default 14); queries SCHEDULED posts in window via Prisma; calls `detectCalendarGaps`; returns `{windowDays, ...result}`
- [x] Add `suggestGapContent(gapDates, platforms, tone?, recentContext?)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` with prompt caching on system block; returns `{suggestions: {date, content, platform, reasoning}[]}` or null when AI not configured
- [x] `POST /api/ai/fill-gaps` endpoint — auth + rate limit (two-arg apiLimiter) + zod validation (`gapDates: string[] 1-7`, `platforms: string[] min 1`, `tone?: string`); fetches last 5 PUBLISHED posts for context; calls `suggestGapContent`; returns `{suggestions}`
- [x] `CalendarGapFillerDialog` component (`src/components/calendar-gap-filler-dialog.tsx`) — "Fill Gaps" button opens dialog; on open fetches `GET /api/analytics/calendar-gaps?windowDays=14`; shows gap summary (count badge, gapRate, longestStreakDays, date badges); 6 platform toggle chips (Facebook/Instagram/Threads/Twitter/LinkedIn/Bluesky); "Generate AI Suggestions" calls fill-gaps endpoint; suggestion cards (toggle accepted, date badge, platform badge, reasoning); "Create N Draft Posts" batch-creates accepted suggestions via `POST /api/posts`
- [x] Update calendar page (`src/app/(dashboard)/calendar/page.tsx`) — import and add `<CalendarGapFillerDialog />` to header button group alongside CalendarExport and CalendarPlannerDialog
- [x] Unit tests for `detectCalendarGaps` utility and `GET /api/analytics/calendar-gaps` endpoint (all gaps when no posts, zero gaps when fully covered, single gap in middle, weekend detection, longestStreakDays calculation, gapRate calculation, GET 401, GET 429, GET 400 invalid windowDays, GET 200 valid shape — 13 tests)

### Phase 284: URL-to-Post Content Generator
- [x] Web content extraction utility (`src/lib/web-content.ts`) — server-side fetch of a URL; strips `<script>`, `<style>`, and HTML tags; extracts `<title>` and text body; returns `{title, content}` or null on network/parse error; max 20,000 chars of body text
- [x] Add `generatePostFromUrl(urlTitle, urlContent, platforms)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` with prompt caching on system block; returns `{posts: {platform: string, content: string}[]}` or null when AI not configured
- [x] `POST /api/ai/post-from-url` route — auth + rate limit + AI check + zod validation (`url: z.string().url()`, `platforms: z.array(z.string()).min(1)`); fetches URL content via `extractWebContent`; calls `generatePostFromUrl`; returns `{posts}`; 503 when AI not configured; 422 when URL content cannot be extracted
- [x] `UrlToPostDialog` component (`src/components/url-to-post-dialog.tsx`) — modal with URL input (validated), "Fetch & Generate" button, per-platform content cards with character count, copy-to-clipboard, and "Use in Composer" button; regenerate option; loading state
- [x] "From URL" button (`Globe` icon) in post composer toolbar — opens `UrlToPostDialog`; `onApply` sets post content; button is NOT disabled when content is empty (generates new content from scratch)
- [x] Unit tests for `POST /api/ai/post-from-url` (auth 401, rate limit 429, AI disabled 503, invalid JSON 400, missing URL 400, invalid URL 400, URL extraction fails 422, success shape with posts array, post fields platform+content, AI error 500 — 10 tests)

### Phase 285: Content Performance Heatmap & Heat Score Analytics
- [x] `GET /api/analytics/performance-heatmap` endpoint — auth + rate limit + `?year=YYYY&metric=score|likes|comments|shares|reach` params; queries PUBLISHED PublishResult records with PostInsights for the year; groups by UTC publishedAt date; computes avg metric per day (weighted score = likes×3+comments×5+shares×4+reach×1+impressions×0.5); returns full 365/366-day array with zeros; exports `PerformanceHeatmapDay` and `PerformanceHeatmapResponse` interfaces
- [x] `PerformanceHeatmapCard` component (`src/components/performance-heatmap-card.tsx`) — GitHub contribution-style 52×7 heatmap grid with blue gradient (bg-muted=0, blue-100≤25%, blue-300≤50%, blue-500≤75%, blue-700>75%); metric selector pills (score/likes/comments/shares/reach); year prev/next navigation; hover tooltip with date/value/postCount; month labels; day-of-week labels; legend
- [x] Integrate `PerformanceHeatmapCard` into analytics dashboard below `PostingHeatmapCard`; add `"performance_heatmap"` widget key and label `"Content Performance Heatmap"` to `WIDGET_KEYS` and `WIDGET_LABELS` in dashboard-widgets route; wrap with `isVisible("performance_heatmap")` guard
- [x] Unit tests for `GET /api/analytics/performance-heatmap` (auth, rate limit, invalid year, invalid metric, defaults to current year/score, 365 days non-leap, 366 days leap, zero values, date aggregation, maxValue, null publishedAt skipped, DB error — 13 tests)

### Phase 286: AI-Powered Influencer Outreach Message Generator
- [x] Add `InfluencerOutreachResult` interface + `generateInfluencerOutreach(influencerName, handle, platform, followerCount, niche, campaignBrief, tone)` to `src/lib/ai.ts` — calls `claude-haiku-4-5` to generate personalized outreach messages; uses prompt caching on system block; returns `{subject, emailBody, dmMessage, followUpMessage}` or null when AI not configured
- [x] `POST /api/ai/influencer-outreach` endpoint — auth + rate limit + AI check + zod validation (`influencerName: string`, `handle: string`, `campaignBrief: string min 10 max 2000`, `platform?: string`, `followerCount?: number`, `niche?: string max 100`, `tone?: "professional"|"friendly"|"casual" default "friendly"`); calls `generateInfluencerOutreach`; returns `{outreach}`; 503 when AI not configured; 500 when function returns null
