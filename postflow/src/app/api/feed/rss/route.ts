...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const feedUrl = `${req.nextUrl.origin}/api/feed/rss`;
    const rssPosts = posts.map((p) => ({
      id: p.id,
      content: p.content,
    ...
    const xml = generateRssFeed(rssPosts, authorName, feedUrl);
    ...
    apiLogger.error({ error }, "feed rss GET error");
