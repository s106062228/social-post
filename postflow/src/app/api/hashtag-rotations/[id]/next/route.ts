...
export async function POST(
  ...
  try {
    ...
    // The group at currentIndex is what we return (the "current" before advancing)
    const currentGroupId = rotation.groupIds[rotation.currentIndex] ?? rotation.groupIds[0];

    // Advance to next index (wraps around)
    const nextIndex = (rotation.currentIndex + 1) % rotation.groupIds.length;

    await prisma.hashtagRotation.update({
      where: { id },
    ...
    return NextResponse.json({
      usedIndex: rotation.currentIndex,
      nextIndex,
