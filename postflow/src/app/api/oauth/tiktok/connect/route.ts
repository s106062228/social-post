...
import { buildTikTokOAuthUrl } from "@/lib/auth/tiktok-oauth";
 ...
 *
 * Initiates the TikTok OAuth 2.0 flow. Requires an authenticated session.
 * Generates a CSRF state token, stores it in an httpOnly cookie, and redirects
 * the browser to the TikTok OAuth consent dialog.
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const oauthUrl = buildTikTokOAuthUrl(state);
    ...
    oauthLogger.error({ err }, `TikTok connect error: ${message}`);
