...
import { buildPinterestOAuthUrl } from "@/lib/auth/pinterest-oauth";
import { oauthLimiter, rateLimitHeaders } from "@/lib/rate-limit";
import { oauthLogger } from "@/lib/logger";

/**
 * GET /api/oauth/pinterest/connect
 *
 * Initiates the Pinterest OAuth 2.0 flow. Requires an authenticated session.
 * Generates a CSRF state token, stores it in an httpOnly cookie, and redirects
 * the browser to the Pinterest OAuth consent dialog.
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    oauthLogger.error({ err }, `Pinterest connect error: ${message}`);
