...
import {
  verifyMastodonToken,
  serializeMastodonToken,
} from "@/lib/auth/mastodon-oauth";
 ...
 *
 * Accepts a Mastodon instance URL and access token, verifies via the instance API,
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const { accountId, username } = await verifyMastodonToken(
      normalizedInstance,
      accessToken
    );

    const tokenData = serializeMastodonToken({
    ...
    // Mastodon access tokens do not expire by default
    ...
    oauthLogger.error({ err }, `Mastodon connect error: ${message}`);
